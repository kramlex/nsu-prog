//
// Created by markd on 20.05.2019.
//
#include <stdio.h>
#include <stdbool.h>
#include <stdlib.h>
...
// Вершины V ; |V| = m*n
// Ребра E ; |E|~= 4*m*n
// (x,y) - координаты 'x' - стартовой позиции
// min_path - ребра в кротчайшем пути от (x,y) до любой из финальных
// Время работы програмы ~ O(|V|+|E|) ~ O(m*n + 4*m*n) ~ O(5*m*n) ~ O(m*n) ~ O(|min_path|)
// Память ~ O(|min_path|)
...
// Ребра E ; |E|~= 4*m*n
// (x,y) - координаты 'x' - стартовой позиции
// Время работы програмы ~ O(|V|+|E|) ~ O(m*n + 4*m*n) ~ O(5*m*n) ~ O(m*n)
// Требуемая память O(4*min(abs(n - x),abs(m - y))) ~ O( min(abs(n - x),abs(m - y)) )
